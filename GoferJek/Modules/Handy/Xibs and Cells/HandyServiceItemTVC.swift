...
class HandyServiceItemTVC: GroupTableViewCell {
    ...
    func populate(with item : ServiceItem,checkOutBookingVC : HandyCheckOutBookingVC,isLast : Bool){
        ...
        self.addItemBtn.addAction(for: .tap) {
            if item.selectedQuantity < item.maximumQuantity{
        ...
        self.addRemoveItemStack.isHidden = item.priceType != .fixed || item.maximumQuantity == 1
        ...
        case .fixed:
            self.addRemoveItemStack.isHidden = item.maximumQuantity <= 1
        case .hourly:
    ...
    func checkButtonStatus(for item : ServiceItem){
        ...
        self.addItemBtn.setActive(item.selectedQuantity != item.maximumQuantity)
    }
}
