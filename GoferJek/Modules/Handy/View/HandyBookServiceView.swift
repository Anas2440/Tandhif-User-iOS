...
class HandyBookServiceView: BaseView {
    ...
    func initView() {
        ...
        if self.priceType == PriceType.fixed && self.bookServiceVC.serviceItem.maximumQuantity > 1 {
...
//                welf.serviceTypeDescriptionLbl.numberOfLines = 1
    ...
    func checkButtonStatus(){
       ...
       self.plusBtn.setActive(itemCount != self.bookServiceVC.serviceItem.maximumQuantity)
    ...
    private func itemCountChange(addOrRemoveItem : Bool) {
        if addOrRemoveItem{
            if itemCount < self.bookServiceVC.serviceItem.maximumQuantity{
                itemCount += 1
            }
        }else{
            if itemCount > 1{
        ...
        let totalPrice = Double(itemCount) * (Double(self.bookServiceVC.serviceItem.baseFare) )
