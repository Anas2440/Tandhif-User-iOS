...
//  Gofer
//
//  Created by Trioangle Technologies on 08/01/19.
...
class ChatInteractor : ChatInteractorProtocol{
    ...
    func initialize(withTrip tripId : String,type:ConversationType){
        ...
        self.trip_id = tripId
        self.conType = type
        UserDefaults.set(Int(tripId), for: .current_trip_id)
        self.chatRef = FireBaseNodeKey.trip_chat.ref(forID: self.trip_id, conType: self.conType)
    ...
    func deinitialize(){
        if self.isInitialized{
            ...
            UserDefaults.removeValue(for: .current_trip_id)
        }
    }
    ...
    func postLocalNotification(WithChat chat: ChatModel){
       // return 
        ...
        let sender_name = UserDefaults.standard.string(forKey: TRIP_DRIVER_NAME) ?? LangCommon.driver
