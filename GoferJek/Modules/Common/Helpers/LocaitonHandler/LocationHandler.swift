...
class LocationHandler : NSObject{
    ...
    var lastKnownLocation : CLLocation?{
        didSet{
            debug(print: "\(self.lastKnownLocation!)")
...
extension LocationHandler : LocationHandlerProtocol{
    ...
    func startListening(toLocationChanges listen : Bool){
        self.backGroundThread = UIApplication
            .shared
            .beginBackgroundTask(withName: "Location", expirationHandler: {
...
//        }
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        print("location : \(locations.last?.description ?? "")")
        guard let newLocation = locations.last,//first!
            newLocation.isValid,
            self.isValidLocation(newLocation) else{return}
