...
final class ConnectionHandler : NSObject {
    ...
    func getRequest(forAPI api: String,
        ...
        parameters["token"] = preference.string(forKey: USER_ACCESS_TOKEN)
        parameters["user_type"] = Global_UserType
        parameters["device_id"] =  strDeviceToken//Constants().GETVALUE(keyname: USER_DEVICE_TOKEN) //strDeviceToken
        parameters["device_type"] = strDeviceType
    ...
    func uploadRequest(for api : APIEnums,
        ...
        let responseHandler = APIResponseHandler()
        var param = params
        param["token"] = preference.string(forKey: USER_ACCESS_TOKEN)
        param["user_type"] = Global_UserType
