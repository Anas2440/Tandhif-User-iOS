    ...
    func initGestures(){
        self.homeLocationView.addAction(for: .tap) { [weak self] in
            if userDefaults.string(forKey: USER_ID) == "10164" {
                ...
                self?.usingPinToGetLocation = false
                let home = Constants().GETVALUE(keyname: USER_HOME_LOCATION)
                if home.isEmpty{
                    self?.setLocationVC.navigateToAddLocation(for: .Home)
                } else {
                    let lat = Double(Constants().GETVALUE(keyname: USER_HOME_LATITUDE)) ?? .zero
                    let lng = Double(Constants().GETVALUE(keyname: USER_HOME_LONGITUDE)) ?? .zero
        ...
        self.workLocationView.addAction(for: .tap) { [weak self] in
            if userDefaults.string(forKey: USER_ID) == "10164" {
                ...
                self?.usingPinToGetLocation = false
                let work = Constants().GETVALUE(keyname: USER_WORK_LOCATION)
                if work.isEmpty {
                    self?.setLocationVC.navigateToAddLocation(for: .Work)
                } else {
                    let lat = Double(Constants().GETVALUE(keyname: USER_WORK_LATITUDE)) ?? .zero
                    let lng = Double(Constants().GETVALUE(keyname: USER_WORK_LONGITUDE)) ?? .zero
