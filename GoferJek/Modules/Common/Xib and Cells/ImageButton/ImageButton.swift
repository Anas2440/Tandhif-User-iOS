...
//        self.centerIV.image = UIImage(named: image)?.withRenderingMode(.alwaysTemplate)
    ...
    func customColorsUpdate() {
        self.setTitle(color: self.isDarkStyle ? .DarkModeTextColor : .SecondaryTextColor)
                self.setBackground(color: self.isDarkStyle ? .DarkModeBackground : .SecondaryColor)
        self.setTint(color: self.isDarkStyle ? .white : .SecondaryTextColor)

    }
    
    class func initWithNib() -> ImageButton {
        let nib = UINib(nibName: "ImageButton",
                        bundle: nil)
        return nib.instantiate(withOwner: nil,
                               options: nil)[0] as! ImageButton
    }
