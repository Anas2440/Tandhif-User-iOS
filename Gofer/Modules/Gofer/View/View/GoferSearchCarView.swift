...
class GoferSearchCarView : BaseView ,UIGestureRecognizerDelegate, GMSMapViewDelegate, setLocationDelegate,UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    @IBOutlet weak var btnRequestGofer: UIButton!
    ...
    var searchCarVC : GoferSearchCarVC!
...
//    var polyLineRoute : NSDictionary = NSDictionary()
   // var dictSearchCarList : NSMutableDictionary = NSMutableDictionary()
    ...
    var viewGoferLoader = GoferLoader()
    ...
    var tempProfileModel : ProfileModel? = nil
    ...
    lazy var seatSelectionView : SeatSelectionView = {
        ...
        selectionView.frame = CGRect(x: refFrame.minX,
                                     y: refFrame.minY - self.safeAreaInsets.bottom,
                                     width: refFrame.width,
                                     height: refFrame.height + self.safeAreaInsets.bottom)
...
//        self.waitingChargeLbl.font = UIFont(name: iApp.GoferFont.centuryBold.rawValue, size: 12)
//        self.waitingChargeLbl.textColor = .Title
//        self.changeLocation.titleLabel?.font = UIFont(name: iApp.GoferFont.centuryBold.rawValue, size: 14)
//        self.changeLocation.titleLabel?.textColor = .DarkTitle
//        self.changeLocation.setTitle(self.lang.changeLocation, for: .normal)
//        self.changePaymentBtn.titleLabel?.font = UIFont(name: iApp.GoferFont.centuryBold.rawValue, size: 12)
//        self.changePaymentBtn.setTitleColor(.Title, for: .normal)
//        self.cashlab.font = UIFont(name: iApp.GoferFont.centuryBold.rawValue, size: 12)
//        self.cashlab.textColor = .Title
//        self.promolab.font = UIFont(name: iApp.GoferFont.centuryBold.rawValue, size: 12)
//        self.promolab.textColor = .Title
//        self.btnRequestGofer.cornerRadius = 15
//        self.btnRequestGofer.titleLabel?.font = UIFont(name: iApp.GoferFont.centuryBold.rawValue, size: 14)
//        self.btnRequestGofer.setTitleColor(.Title, for: .normal)
...
//        self.savePrefernceBtn.titleLabel?.font = UIFont(name: iApp.GoferFont.centuryBold.rawValue, size: 14)
...
//    }
    ...
    override func didLoad(baseVC: BaseViewController) {
        ...
        self.searchCarVC = baseVC as? GoferSearchCarVC
        self.darkModeChange(baseVc: baseVC)
...
//        viewRefresh.layer.borderColor = UIColor.black.cgColor
        googleMapView.frame = CGRect(x: 0, y: 0, width: self.frame.size.width, height: self.frame.size.height - viewObjectHolder.frame.size.height)
...
//        viewEditLocHolder.frame = rectView
    ...
    func initLayer(){
        ...
        self.addSubview(self.seatSelectionView)
        self.bringSubviewToFront(self.seatSelectionView)
    ...
    func showSeatSelectionView(_ show : Bool,duration : TimeInterval = 0.6){
        ...
        let safeHeight = self.safeAreaInsets.bottom
        ...
        UIView.animate(withDuration: duration) {
            if show{
                ...
                self.seatSelectionView.transform = CGAffineTransform(translationX: 0,
                                                                     y: self.seatSelectionView.frame.height + self.safeAreaInsets.bottom)
            }
    ...
    func initLanguage(){
        ...
        self.btnRequestGofer.setTitle(self.lang.request + " ...", for: .normal)
    ...
    func payView(){
        ...
        case .stripe:
            ...
            if let last4 : String = UserDefaults.value(for: .card_last_4),
                ...
                paymentimg.image = self.searchCarVC.getCardImage(forBrand: brand)
                paymentimg.tintColor = .ThemeYellow
            }
        case .brainTree:
            cashView.isHidden = false
    ...
    func updatefilterIcon()
    {
        ...
        if let request =  self.searchCarVC.profileModel?.requestOptions.filter({$0.isSelected}).compactMap({$0.id.description}){
            if request.count == 0{
...
//                self.filterButton.setImage(#imageLiteral(resourceName: "filter-on"), for: .normal)
    ...
    @IBAction func filterBtnAction(_ sender: Any) {
        guard let model = self.searchCarVC.profileModel else {return}
        self.tempProfileModel = ProfileModel(copy: model)
        self.preferencePopupView.isHidden = false
        self.filterView.transform = CGAffineTransform(translationX: 0, y: self.frame.size.height)
        ...
        self.addSubview(self.preferencePopupView)
    ...
    @IBAction func goToEditLocation()
    {
        ...
        viewRequest.strPickUpLocation = self.searchCarVC.pickUpLocName
        viewRequest.strDropLocation = self.searchCarVC.dropLocName
        viewRequest.pickUpLatitude = self.searchCarVC.pickUpLatitude
        viewRequest.pickUpLongitude = self.searchCarVC.pickUpLongitude
        viewRequest.dropLatitude = self.searchCarVC.dropLatitude
        viewRequest.dropLongitude = self.searchCarVC.dropLongitude
        viewRequest.scheduledTime = self.searchCarVC.scheduledTime
        self.searchCarVC.navigationController?.pushViewController(viewRequest, animated: true)
    }
    ...
    func onExitSetLocation(from viewController: UIViewController) {
        self.searchCarVC.navigationController?.popViewController(animated: false)
    }
    ...
    internal func onLocationTapped(pickUpLatitude: CLLocationDegrees, pickUpLongitude: CLLocationDegrees, dropLatitude: CLLocationDegrees, dropLongitude: CLLocationDegrees,dropLocName: String, pickUpLocName: String, scheduledTime:String?)
    {
        ...
        self.searchCarVC.pickUpLatitude = pickUpLatitude
        self.searchCarVC.pickUpLongitude = pickUpLongitude
        self.searchCarVC.dropLatitude = dropLatitude
        self.searchCarVC.dropLongitude = dropLongitude
        self.searchCarVC.dropLocName = dropLocName
        self.searchCarVC.pickUpLocName = pickUpLocName
        self.searchCarVC.scheduledTime = scheduledTime
        self.startLoader()
        map?.clear()
        self.searchCarVC.callAPIForSearchNearestCars()
    }
    ...
    func startLoader()
    {
        ...
        viewObjectHolder.addSubview(viewGoferLoader)
        viewGoferLoader.frame = CGRect(x: 0, y: 0, width: viewObjectHolder.frame.size.width, height: 2)
        viewGoferLoader.beginRefreshing()
    }
    ...
    @IBAction func CashAction(_ sender: Any) {
        ...
        self.searchCarVC.navigationController?.pushViewController(tripView, animated: true)
    }
    
    func addSubviewContactAdmin(isRemove:Bool = false) {
        let imageview = UIImageView()
        imageview.frame = CGRect(x: self.frame.width-60, y: self.map!.frame.height - 69, width: 50, height: 50)
    ...
    @objc func callAdmin() {
        self.searchCarVC.checkMobileNumeber(isDirectCall: false)
     }
    ...
    func showRoute()
    {
        if let gPath = self.path {
            ...
            self.searchCarVC.animatePolyLine(pickUpLatitude: self.searchCarVC.pickUpLatitude, pickUpLongitude: self.searchCarVC.pickUpLongitude, dropLatitude: self.searchCarVC.dropLatitude, dropLongitude: self.searchCarVC.dropLongitude)
        }
    }
    ...
    func gotoCarDetailPage(selectedCar: SearchCarsModel,seatCount : Int? = nil)
    {
        ...
        if self.searchCarVC.scheduledTime == nil {
            btnRequestGofer.setTitle(String(format:"\(self.lang.request) %@",strCarType.uppercased()), for: .normal)
            btnRequestGofer.titleLabel?.text = String(format:"\(self.lang.request) %@",strCarType.uppercased())
        }
        else {
            
            for view in btnRequestGofer.subviews {
                view.removeFromSuperview()
            }
            
            btnRequestGofer.setTitle("", for: .normal)
            let title = UILabel(frame: CGRect(x: 1, y: 0, width: btnRequestGofer.frame.size.width, height: (btnRequestGofer.frame.size.height / 2) - 1))
            title.backgroundColor = UIColor.clear
            if let aSize = UIFont(name: "Chalkboard", size: 14) {
                title.font = aSize
            }
            title.font = UIFont(name: AppWebConstants.GoferFont.medium.rawValue, size: CGFloat(14))
            title.text = String(format:"\(self.lang.request) %@",strCarType.uppercased())
            title.textColor = UIColor.white
            title.textAlignment = .center
            btnRequestGofer.addSubview(title)
            let subtitle = UILabel(frame: CGRect(x: 0, y: title.frame.size.height + 1, width: btnRequestGofer.frame.size.width, height: (btnRequestGofer.frame.size.height / 2) - 1))
            subtitle.backgroundColor = UIColor.clear
            if let aSize = UIFont(name: "Helvetica", size: 14) {
                subtitle.font = aSize
            }
            subtitle.font = UIFont(name: AppWebConstants.GoferFont.medium.rawValue, size: CGFloat(10))
            subtitle.text = self.searchCarVC.scheduledTime
            subtitle.textColor = UIColor.white
            subtitle.textAlignment = .center
            btnRequestGofer.addSubview(subtitle)
        }
        ...
        if car.driverIDS.count == 0
        {
            ...
            btnRequestGofer.isUserInteractionEnabled = false
            btnRequestGofer.backgroundColor = UIColor.ThemeInactive
            self.waitingChargeLbl.text = nil
            let tit = self.lang.noCabs
            searchCarVC.appDelegate!.createToastMessage(tit, bgColor: UIColor.ThemeYellow, textColor: UIColor.white)
            self.driverLTManger?.stopUpdating()
        }
        else
        {
            btnRequestGofer.isUserInteractionEnabled = true
            btnRequestGofer.backgroundColor = UIColor.ThemeYellow
...
//            self.displayNearestCarsInMapView(car.arrcCarLocations, car_id: car.car_id)
          ...
          if selectedCar.shareRideEnabled{
              self.btnRequestGofer.setTitle(self.lang.confirmSeats.capitalized,
                                            for: .normal)
          }
...
//        self.polyline.strokeColor = UIColor(red: 0, green: 0, blue: 0, alpha: 0.5)
    ...
    /*
     */
    @IBAction func onRequestGoferTapped(_ sender:UIButton!)
    {
            ...
            if !(UIApplication.shared.isRegisteredForRemoteNotifications)
            {
                ...
                settingsActionSheet.addAction(UIAlertAction(title:self.lang.ok, style:UIAlertAction.Style.cancel, handler:{ action in
                   // self.appDelegate?.registerForRemoteNotification()
                    self.searchCarVC.appDelegate?.pushManager.registerForRemoteNotification()
                }))
                searchCarVC.present(settingsActionSheet, animated:true, completion:nil)
                return
            }
        }
    ...
    func handleSearchCarResponse(_ json : JSON){
        ...
        if json.status_code == 2
        {
            ...
            self.map?.clear()
            self.btnRequestGofer.isUserInteractionEnabled = false
            self.btnRequestGofer.backgroundColor = UIColor.ThemeInactive
        ...
        else if json.status_code == 1
        {
            ...
            if self.availableCars.first?.driverIDS.count ?? 0 > 0 {
                self.btnRequestGofer.isUserInteractionEnabled = true
                self.btnRequestGofer.backgroundColor = UIColor.ThemeYellow
            }else{
                self.btnRequestGofer.isUserInteractionEnabled = false
                self.btnRequestGofer.backgroundColor = UIColor.ThemeInactive
            }
        ...
        else
        {
            ...
            self.cashView.isHidden = true
                ...
                {
                    self.searchCarVC.appDelegate?.createToastMessage(json.status_message, bgColor: UIColor.ThemeYellow, textColor: UIColor.white)
                }
        ...
        self.collectionCarDetails.reloadData()
        self.viewGoferLoader.endRefreshing()
...
//                tempMarker.icon = UIImage(named: "top view")//"gofergo.png")
            }
            else if car_id == "2"
            {
...
//                tempMarker.icon = UIImage(named: "topView2")//"goferx.png")
            }
            else
            {
                let imageView = UIImageView(frame: CGRect(x: 0, y: 0, width: 40, height: 40))
...
//                tempMarker.icon = UIImage(named: "topView3")//"goferxl.png")
            }
            tempMarker.map = map
    ...
    func gotoGettingNearCarView(_ dicts: JSON)
    {
        ...
        let pickUp = CLLocationCoordinate2D(latitude: self.searchCarVC.pickUpLatitude,
                                            longitude: self.searchCarVC.pickUpLongitude)
        
        let drop = CLLocationCoordinate2D(latitude: self.searchCarVC.dropLatitude,
                                            longitude: self.searchCarVC.dropLongitude)
    ...
    func makeRequest(wiht seats : Int? = nil){
        ...
        if self.searchCarVC.scheduledTime == nil {//For normal trips
            var dicts = JSON()
            
            dicts["token"] =  Constants().GETVALUE(keyname: USER_ACCESS_TOKEN)
            dicts["pickup_latitude"] = String(format:"%f",self.searchCarVC.pickUpLatitude)
            dicts["pickup_longitude"] = String(format:"%f",self.searchCarVC.pickUpLongitude)
            dicts["drop_latitude"] = String(format:"%f",self.searchCarVC.dropLatitude)
            dicts["drop_longitude"] = String(format:"%f",self.searchCarVC.dropLongitude)
            dicts["pickup_location"] = String(format:"%@",self.searchCarVC.pickUpLocName)
            dicts["drop_location"] = String(format:"%@",self.searchCarVC.dropLocName)
            ...
            if let _seat = seats{
                ...
                self.searchCarVC.seatCount = _seat.description
            }
            if Shared.instance.isCovidEnabled {
//                let view : CovidAlertVC = CovidAlertVC.initWithStory(params: dicts,navigationCtrl: self.searchCarVC.navigationController ?? UINavigationController(), isSchedule: false)
                let view  = CovidAlertVC.initWithStory(self,bookingType: .bookNow)

//                view.modalPresentationStyle = .overCurrentContext
                self.searchCarVC.navigationController?.present(view, animated: true, completion: nil)
            }else{
            ...
            dict["pickup_latitude"] = String(format:"%f",self.searchCarVC.pickUpLatitude)
            dict["pickup_longitude"] = String(format:"%f",self.searchCarVC.pickUpLongitude)
            dict["drop_latitude"] = String(format:"%f",self.searchCarVC.dropLatitude)
            dict["drop_longitude"] = String(format:"%f",self.searchCarVC.dropLongitude)
            dict["pickup_location"] = String(format:"%@",self.searchCarVC.pickUpLocName)
            dict["drop_location"] = String(format:"%@",self.searchCarVC.dropLocName)
            ...
            if Shared.instance.isCovidEnabled {
//                let view : CovidAlertVC = CovidAlertVC.initWithStory(params: dict,navigationCtrl: self.searchCarVC.navigationController ?? UINavigationController(), isSchedule: true)
//                view.modalPresentationStyle = .overCurrentContext
//                view.scheduledTime = self.searchCarVC.scheduledTime
                let view = CovidAlertVC.initWithStory(self, bookingType: .bookLater)
                self.searchCarVC.navigationController?.present(view, animated: true, completion: nil)
            }else{
                self.goToScheduleScreen(dicts: dict, scheduletime: self.searchCarVC.scheduledTime ?? "")
            }
        ...
        if selected.apply_peak{
            ...
            let scheduleDetailVC = ScheduleRideDetailViewController
                .initWithStory(params: dicts,
                               ...
                               scheduledTimeString: scheduletime,
                               path: gPath)
            self.searchCarVC.navigationController?.pushViewController(scheduleDetailVC, animated: true)
        }
    }
    ...
    func onCreateMapMarker()
    {
        ...
        pickupMarker.position = CLLocationCoordinate2D(latitude: self.searchCarVC.pickUpLatitude, longitude: self.searchCarVC.pickUpLongitude)
...
//        pickupMarker.icon = UIImage(named: "circle")
        ...
        dropMarker.position = CLLocationCoordinate2D(latitude: self.searchCarVC.dropLatitude, longitude: self.searchCarVC.dropLongitude)
...
//        dropMarker.icon = UIImage(named: "box")
        ...
        let vancouver = CLLocationCoordinate2DMake(self.searchCarVC.pickUpLatitude, self.searchCarVC.pickUpLongitude)
        let calgary = CLLocationCoordinate2DMake(self.searchCarVC.dropLatitude, self.searchCarVC.dropLongitude)
...
//        return CGSize(width: (rect.size.width/3)-20, height: 145.0)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath)
    {
        ...
        if carTag == currentTag
        {
            ...
            searchCarVC.present(viewFare, animated: false, completion: nil)
            return
        }
        else
        {
...
extension GoferSearchCarView : paymentMethodSelection{
    func updateContent() {
...
extension GoferSearchCarView : SeatSelectionDelegate{
...
extension GoferSearchCarView : UITableViewDelegate,UITableViewDataSource{
...
extension GoferSearchCarView : CovidDelegate {
    ...
    func wsToBookNow() {
        ...
        if self.searchCarVC.seatCount != ""{
            dicts["seat_count"] = self.searchCarVC.seatCount
        }
        self.gotoGettingNearCarView(dicts)
    ...
    func wsToBookLater() {
        ...
        self.goToScheduleScreen(dicts: dicts, scheduletime: self.searchCarVC.scheduledTime ?? "")
    }
}
