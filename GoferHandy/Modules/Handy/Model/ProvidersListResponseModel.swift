...
class ProviderResponse: Codable {
    ...
    var userLocation: CLLocation {
        return .init(latitude: userLatitude ?? 0.0,
    ...
    required init(from decoder: Decoder) throws {
        ...
        self.statusCode = container.safeDecodeValue(forKey: .statusCode)
        self.provider = try container.decode(Provider.self, forKey: .provider)
        self.userLocationName = try? container.decodeIfPresent(String.self, forKey: .userLocationName)
        self.userLatitude = try? container.decodeIfPresent(Double.self, forKey: .userLatitude)
        self.userLongitude = try? container.decodeIfPresent(Double.self, forKey: .userLongitude)
    }
}

class ProviderWrapperResponse: Codable {
    let statusCode: String
    ...
    required init(from decoder: Decoder) throws {
        ...
        do {
            ...
            print("❌ Failed to decode 'statusCode': \(error)")
            throw error
        }
        ...
        do {
            ...
            print("❌ Failed to decode 'providerDetails': \(error)")
            throw error
        }
        ...
        do {
            ...
            print("❌ Failed to decode 'statusMessage': \(error)")
            throw error
        }
    }
