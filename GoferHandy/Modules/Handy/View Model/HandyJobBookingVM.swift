...
//        }
    ...
    func wsToGetProviderDetail(for service : Service? = nil,
                               ...
                               _ result : @escaping Closure<Result<ProviderWrapperResponse,Error>>){
        ...
        self.connectionHandler?
            .getRequest(for: .getProviderDetail,
                        params: params)
            .responseDecode(to: ProviderWrapperResponse.self,
                            { (response) in
                print(response)
                Shared.instance.removeLoaderInWindow()
                result(.success(response))
    ...
    func wsToBook(provider : Provider?,
        ...
        params["is_wallet"] = Constants().GETVALUE(keyname: "selectwallet")
