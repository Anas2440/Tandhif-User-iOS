...
protocol CallManagerDelegate {
    /**
     variable to return client is initialized or not
     - Author: Abishek Robin
     */
    var isInitialized : Bool { get }
    
    /**
     initialize client for specific user
     - Author: Abishek Robin
     - Parameters:
       - environment: sandbox or live
       - user: client's user id
     - Warning: throws reason if can't initialize client
     */
    func initialize(environment: CallManager.Environment, for user: String) throws
    ...
    /**
     configure whether to receive calls or not
     - Author: Abishek Robin
     - Parameters:
       - waitForCall: should receive call or not
     - Warning: call after initializing client
    ...
    /**
     didReceive push notification from server for call
     - Author: Abishek Robin
     - Parameters:
       - data: push data
     */
    ...
    /// The client instance. No longer tied to Sinch. Type is generic.
    var clientInstance: Any? { get set }
}
