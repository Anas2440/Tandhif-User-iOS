...
//  Gofer
//
//  Created by Trioangle Technologies on 19/12/18.
...
//    }
        func setStatusBarStyle(_ style: UIStatusBarStyle) {
            if #available(iOS 13.0, *){
                ...
                view?.backgroundColor = style == .lightContent ? UIColor.PrimaryColor : .white
    //            view?.setValue(style == .lightContent ? UIColor.white : .ThemeMain, forKey: "foregroundColor")
                ...
                statusBar.backgroundColor = style == .lightContent ? UIColor.PrimaryColor : .white
    //            statusBar.setValue(style == .lightContent ? UIColor.white : .ThemeMain, forKey: "foregroundColor")
            }
    }
...
extension UIViewController{
    ...
    func ThemeChangeForAlert() {
        ...
        if let alertView : DeletePHIAlertview  = window?.subviews.filter({$0 is DeletePHIAlertview}).last as? DeletePHIAlertview {
            alertView.centerView.backgroundColor = alertView.isDarkStyle ?  .DarkModeBackground : AlertTheme.Background.color
            alertView.message_titleLb.textColor = alertView.isDarkStyle ?  .DarkModeTextColor : AlertTheme.Title.color
            alertView.messageDescriptionLbl.textColor = alertView.isDarkStyle ?  .DarkModeTextColor : AlertTheme.Message.color
            alertView.deletePHI_popupTwoOption.backgroundColor = alertView.isDarkStyle ?  .DarkModeBackground : AlertTheme.Background.color
            alertView.deletePHI_popupOneOption.backgroundColor = alertView.isDarkStyle ?  .DarkModeBackground : AlertTheme.Background.color
        }
    }
...
//                .init(title: option.replacingOccurrences(of: "ð", with: ""),
//                      style: option.contains("ð") ? .destructive : .default,
//                      handler: { (action) in
//                completion(index)
//            }))
//        }
