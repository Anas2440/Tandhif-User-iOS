...
extension UILabel{
    ...
    func setText(_ fullText : String,withLinks links : [HyperLinkModel]){
        ...
        let attrbute : [NSAttributedString.Key : Any] = [.foregroundColor : self.isDarkStyle ? UIColor.DarkModeTextColor :  UIColor.SecondaryTextColor,
                                                         .font : AppTheme.Fontmedium(size: 13).font]
        let nsStr = fullText as NSString
        let textRange = NSMakeRange(0, nsStr.length)
        full_attr_str.addAttributes(attrbute, range: textRange)
        let highlight = UIColor.ThemeTextColor //UIColor(hex: "#1c5c9a")
