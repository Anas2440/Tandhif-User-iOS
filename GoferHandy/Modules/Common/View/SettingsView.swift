...
//    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section != 0 {
            ...
            if indexPath.row == 0 && Constants().GETVALUE(keyname: "home_loc").count > 0 {
                cell.lblTitle?.text = LangCommon.home
                cell.lblSubTitle?.isHidden = false
                cell.lblSubTitle?.text = Constants().GETVALUE(keyname: "home_loc")
            } else if indexPath.row == 0 {
                cell.lblSubTitle?.isHidden = true
                cell.lblTitle?.text = viewController.arrTitle[indexPath.row]
            }
            
            if indexPath.row == 1 && Constants().GETVALUE(keyname: "work_loc").count > 0 {
                 cell.lblTitle?.text = LangCommon.work
                cell.lblSubTitle?.isHidden = false
                cell.lblSubTitle?.text = Constants().GETVALUE(keyname: "work_loc")
