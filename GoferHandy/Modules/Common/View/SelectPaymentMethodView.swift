...
class SelectPaymentMethodView : BaseView {
    ...
    func willAppear(baseVC: BaseViewController) {
        ...
        self.wallectamount = Constants().GETVALUE(keyname: "wallet_amount")
...
//        self.selectPaymentMethodVC.wsToGetOptionList()
    ...
    func generateTableDataSource(with paymentList : PaymentList){
        ...
        if self.selectPaymentMethodVC.canShowWallet {
            ...
            walletData.isSelected = Constants().GETVALUE(keyname: "selectwallet") == "Yes"
        ...
        if self.selectPaymentMethodVC.canShowPromotion {
            ...
            if Constants().GETVALUE(keyname: "Promo_Code") != "" &&
                Constants().GETVALUE(keyname: "Promo_Code") != "0" {
...
//                cell.promoCountLabel.text = Constants().GETVALUE(keyname: "Promo_Code")
...
extension SelectPaymentMethodView : UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case LangCommon.wallet :
            if Constants().GETVALUE(keyname: "selectwallet") == "Yes" {
                Constants().STOREVALUE(value: "No" , keyname: "selectwallet")
            } else {
                Constants().STOREVALUE(value: "Yes" , keyname: "selectwallet")
            }
