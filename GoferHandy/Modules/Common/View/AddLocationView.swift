...
extension AddLocationView : UITextFieldDelegate {
    ...
    func startAutoComplete(_ countdownTimer: Timer) {
        ...
        self.dataLoader?.sendAutocompleteRequest(searchString: newSearch, location: nil)
    }
...
extension AddLocationView : GooglePlacesDataLoaderDelegate {
    ...
    func googlePlacesDataLoader(_ loader: GooglePlacesDataLoader, didLoadPlaceDetails details: [String: Any], withAttributions attributions: String?) {
        self.searchDidComplete(withPlaceDetails: details, andAttributions: attributions!)
    }
    
    func googlePlacesDataLoader(_ loader: GooglePlacesDataLoader, didFailWithError error: Error) {
    ...
    func searchDidComplete(withPlaceDetails placeDetails: [AnyHashable: Any], andAttributions htmlAttributions: String) {
        guard let placeDict = placeDetails as? [String: Any] else {
            print("Invalid placeDetails format")
            return
        }

        locationAnnotation = PlaceDetailsAnnotation(placeDetails: placeDict)
        
        selectedLocation.searchedAddress = placeDict["formatted_address"] as? String

        if let coordinate = locationAnnotation?.coordinate {
            selectedLocation.longitude = String(format: "%f", coordinate.longitude)
            selectedLocation.latitude = String(format: "%f", coordinate.latitude)
            selectedLocation.currentLocation = CLLocation(latitude: coordinate.latitude, longitude: coordinate.longitude)
        }
    }
...
extension AddLocationView : UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView,
        ...
        if self.autocompletePredictions.count == 0 {
            ...
            let titleString: String? = (adict["description"] as? String)
...
//                    LocationManager.instance.isAuthorized else {return}
            ...
            let title  = adict["description"] as? String
...
protocol GooglePlacesDataLoaderDelegate: AnyObject {
    func googlePlacesDataLoader(_ loader: GooglePlacesDataLoader, didLoadAutocompletePredictions predictions: [Any])
    func googlePlacesDataLoader(_ loader: GooglePlacesDataLoader, didLoadPlaceDetails details: [String: Any], withAttributions attributions: String?)
    func googlePlacesDataLoader(_ loader: GooglePlacesDataLoader, didFailWithError error: Error)
}
...
class GooglePlacesDataLoader {
    ...
    weak var delegate: GooglePlacesDataLoaderDelegate?

    // MARK: - Initializer

    init(delegate: GooglePlacesDataLoaderDelegate?) {
    ...
    /// - Parameters:
    ///   - searchString: The query string for autocomplete
    ///   - location: Location bias for the search (optional)
    func sendAutocompleteRequest(searchString: String, location: CLLocation?) {
        // Implementation goes here
        // Call delegate?.googlePlacesDataLoader(self, didLoadAutocompletePredictions: results) on success
        // Or delegate?.googlePlacesDataLoader(self, didFailWithError: error) on failure
    }

    /// Sends a request to the Google Place Details API
    /// - Parameter referenceID: The reference ID of the place
    func sendDetailRequest(referenceID: String) {
        // Implementation goes here
        // Call delegate?.googlePlacesDataLoader(self, didLoadPlaceDetails: details, withAttributions: attributions) on success
        // Or delegate?.googlePlacesDataLoader(self, didFailWithError: error) on failure
    }
...
class PlaceDetailsAnnotation: NSObject, MKAnnotation {

    // MARK: - Properties

    let placeDetails: [String: Any]

    // These are required by MKAnnotation
    ...
    init(placeDetails: [String: Any]) {
        self.placeDetails = placeDetails

        // Extract coordinate, title, and subtitle from placeDetails dictionary
        if let lat = placeDetails["latitude"] as? CLLocationDegrees,
           let lon = placeDetails["longitude"] as? CLLocationDegrees {
        ...
        self.subtitle = placeDetails["address"] as? String
...
class CurrentLocationAnnotation: NSObject, MKAnnotation {
    ...
    // MARK: - MKAnnotation Requirement
    var coordinate: CLLocationCoordinate2D

    // Optional annotation title/subtitle
    var title: String?
    var subtitle: String?
